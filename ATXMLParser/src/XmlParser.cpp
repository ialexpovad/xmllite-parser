    ...
    while ( m_pIXmlReader->Read( &NodeType ) == S_OK )
    {
        switch ( NodeType )
        {
            ...
            case XmlNodeType_Text:
                if (FAILED(hr = m_pIXmlReader->GetLocalName(&pszName, nullptr)))
                {
                    _tprintf(_T("XmlText Error getting name: %08.8lx \n"), hr);
                    return false;
                }
                //m_pHandler->OnCharacterData(pszValue, wcslen(pszValue));
                if (FAILED(hr = m_pIXmlReader->GetValue(&pszValue, nullptr)))
                {
                    _tprintf(_T("XmlText Error getting value: %08.8lx \n"), hr);
                    return false;
                }
                m_pHandler->OnCharacterData(pszName, pszValue, wcslen(pszValue));
